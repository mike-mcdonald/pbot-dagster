...
@op(
    config_schema={
        "driver_co": Field(String, description="Uber or Lyft")
    },
    out=Out(
        List[String],
    ),
...
def get_other_files(context: OpExecutionContext, allfiles: list[str]):
    ...
    context.log.info(f" 🚗 Get other files  for {driver_co}: {len(other)} in {datetime.now() - trace}.")
    return other
            ...
            for localfile in bgc_list:
                ...
                def analyze(localfile):
                    ...
                    if match_filename is None:
                        context.log.info(f" 🚗 filename does not match _BGC, MVR or DMV.pdf")
                    status = False
                    analysis = []
                    reportdate = None
...
def download_all(context: OpExecutionContext, list_of_list: list[list[str]]):
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        context.log.info(f"Downloading {len(downloads)} files took {datetime.now() - trace}")
        context.log.info(
            f" 🚗 Local files: {files} "
        )
    return files
...
def upload_all(context: OpExecutionContext, local_files: list[str]):
    ...
    context.log.info(f"Uploading {len(local_files)} files took {datetime.now() - trace}")
