...
def fetch_reports(context: OpExecutionContext):
    ...
    if response.status_code == 200:
        ...
        with path.open(path, "w").write(data)
    
        context.log.info(f"🚀 Fetch reports success.")
    else:
        context.log.info(f"🚀 Fetch reports error '{response.status_code}' - '{response.text}'...")
...
def read_reports(context: OpExecutionContext, path: str) -> Tuple[dataframe, bool, str, int, str]:

    p = Path(path)
    with open(p, "r") as file:
        data = json.load(file)

    abautos_details_key_value = 17698062540823    #this indicates it is an abautos zendesk report
    ...
    for report_record in data["results"]:
        ...
        find_occupied =""
        find_occupied = [r for r in custom_fields if r["id"] == abautos_occupied_key_value ][0][
                "value"
            ]
        find_occupied_record = [r for r in custom_fields if r["id"] == abautos_occupied_key_value ][0]["id"]
        ...
        checkaddr =str(abautos_report_fields['report_location:location_address']["value"])
        ...
        if 'report_vehicle:license_plate_state' in abautos_report_fields:
            vehicleState = abautos_report_fields['report_vehicle:license_plate_state']["value"].upper()
        if 'report_vehicle:license_plate_number' in abautos_report_fields:
            vehicleLicense  = abautos_report_fields['report_vehicle:license_plate_number']["value"].upper()
        ...
        if 'report_vehicle_inoperable' in abautos_report_fields:   
            inoperables = str(abautos_report_fields['report_vehicle_inoperable']["value"]).replace('[','').replace(']','').replace('\'','')
        ...
        if 'report_location_is_private' in abautos_report_fields:       
            isprivate = abautos_report_fields['report_location_is_private']["value"].strip()
            if len(isprivate)> 0: 
                details = details + "Private:" + isprivate +" "
            #print("Isprivate : "+ isprivate )

        if 'report_location:location_details' in abautos_report_fields: 
            locdetails = str(abautos_report_fields['report_location:location_details']["value"]).strip().replace('[','').replace(']','').replace('\'','')
            if len(locdetails)> 0: 
        ...
        if 'report_location:location_attributes' in abautos_report_fields: 
            locattr = str(abautos_report_fields['report_location:location_attributes']["value"]).strip().replace('[','').replace(']','').replace('\'','')
            if len(locattr)> 0: 
        ...
        if len(details) <= max_size:
            ...
            vehicleDescription = textwrap.wrap(details, max_size-3)[0] + "..."

 
 
        create_case = (","+ "\'" + vehicleColor + "\'"+ ","+ "\'" 
        + vehicleType + "\'"+ "," + "\'" +  vehicleMake + "\'" + ","+ "\'" 
        + vehicleLicense + "\'"+ ","+ "\'" +  vehicleState + "\'" + ","+ "\'"
        + vehicleDescription + "\'"+ ","+ "\'" + vehicleArea + "\'"+ ","+ "\'" +  vehicleAddress + "\'"+ ","
        + latitude +  ","+   longitude + ","+ "\'"
        + contactFirstName + "\'"+ ","+ "\'"  + contactLastName +"\'"+ ","+ "\'"       
        + contactPhone + "\'"+ ","+ "\'"  + contactEmail + "\'"+ ","
        + str(waiveConfid) +  ","+ "\'" +  occupied + "\'" 
        )
    ...
    return (df,has_more_data,next_url,count_reports,"Read returns "+count_reports+ " reports. Has more data is "+has_more_data+"." )

@op(
ins={
    "zendesk_data": In(dataframe),
},
out={
    "output_message": Out(str) 
    },
)
def write_reports(context: OpExecutionContext, zendesk_data: dataframe):
        ...
        for index, row in zendesk_data.iterrows():
            ...
            for results in cursor.fetchall():
                if results[0]== 0:  
                    ...
                    queryCreate =("Exec sp_CreateAbCaseZendesk " +  row['zendesk_id'] +  row['attributes'])
                    print(queryCreate)
                    cursor.execute(queryCreate) 
                    for results in cursor.fetchall():
                        print(results)    
                        if 'Success' in results[1]:
                            context.log.info(f"🚀 Write reports successfully with Abandoned Autos abcaseid: - '{str(results[0])}'.")
                        else:
                            context.log.info(f"🚀 Write reports failed for Zendesk ID: - '{row['zendesk_id']}'.")
                else:
                    context.log.info(f"🚀 Zendesk Abandoned Autos case already exists - abcaseid: - '{str(results[0])}'.")      
        ...
        run_config={
            ...
            "ops": {
                "process_zendesk_data": {
                     "inputs": {"api_endpoint": "https://portlandoregon.zendesk.com/api/v2/search/export?page[size]=1&filter[type]=ticket&query=group_id:18716157058327&ticket_form_id:17751920813847&created>"+
"${execution_date}"},
                },
            },
        },
