...
def custom_data_frame_type_check(_, value):
    if not isinstance(value, list):
        return TypeCheck(
            success=False,
            description=(
                "SimpleDataFrame should be a list of dicts, got "
                "{type_}"
...
def find_highest_calorie_cereal(_, cereal):
    sorted_cereals = list(
        sorted(cereal, key=lambda cereal: cereal["calories"])
    )
...
def find_highest_protein_cereal(_, cereal):
    sorted_cereals = list(
        sorted(cereal, key=lambda cereal: cereal["protein"])
    )
    return sorted_cereals[-1]["name"]
...
def display_results(context, most_calories, most_protein, sugariest):
    ...
    context.log.info(
        f"Today is {date}. Most protein-rich cereal: {most_protein}")
